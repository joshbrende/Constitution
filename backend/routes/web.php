...
Route::get('/verify-certificate', [App\Http\Controllers\CertificateVerificationController::class, 'show'])
...
Route::middleware('auth')->group(function () {
    ...
    // Academy – show courses; admins get manage link
    Route::get('/academy', function () {
        $courses = \App\Models\Course::withCount(['modules', 'assessments'])
            ->orderByRaw("CASE status WHEN 'published' THEN 0 WHEN 'draft' THEN 1 ELSE 2 END")
            ->orderBy('is_mandatory', 'desc')
            ->orderBy('title')
            ->get();
        $canManage = app(\App\Services\AdminAccessService::class)->canAccessSection(auth()->user(), 'academy');
        return view('sections.academy', compact('courses', 'canManage'));
    })->name('academy.home');
    Route::get('/library', [App\Http\Controllers\WebLibraryController::class, 'index'])->name('library.home');
    Route::get('/library/documents/{document}', [App\Http\Controllers\WebLibraryController::class, 'show'])->name('library.document');

    Route::get('/party', [WebPartyController::class, 'index'])->name('party.home');

    Route::get('/party-organs', [App\Http\Controllers\WebPartyOrgansController::class, 'index'])->name('party-organs.home');
    Route::get('/party-organs/{party_organ}', [App\Http\Controllers\WebPartyOrgansController::class, 'show'])->name('party-organs.show');

    Route::get('/certificate-preview', function () {
        $service = app(\App\Services\CertificatePdfService::class);
        ...
        $cert = (object) [
            ...
            'course' => (object) [
                'title' => $courseTitle,
                'certificate_title' => $certificateTitle ?: 'Certificate of Completion',
            ],
