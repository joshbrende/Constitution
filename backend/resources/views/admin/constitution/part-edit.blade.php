...
@section('title', 'Edit Part – ' . $part->title)
@section('page_heading', 'Edit Part')
    ...
    <div class="dash-content">
        ...
        <section class="dash-panel" style="grid-column: span 2;">
            <div class="dash-panel-header">
                <div>
                    <div class="dash-panel-title">Edit Part</div>
            ...
            <form method="POST" action="{{ route('admin.constitution.parts.update', $part) }}" style="max-width:32rem;">
                @csrf
                @method('PUT')
                <div style="display:grid;gap:1rem;">
                    <div>
                        <label for="number" style="font-size:0.75rem;color:var(--text-muted);display:block;margin-bottom:0.25rem;">Number</label>
                        <input id="number" type="number" name="number" min="1" value="{{ old('number', $part->number) }}" required
                    ...
                    <div>
                        ...
                        <input id="title" type="text" name="title" value="{{ old('title', $part->title) }}" required
                            style="width:100%;padding:0.5rem;border:1px solid var(--border-subtle);border-radius:0.4rem;background:rgba(15,23,42,0.9);color:var(--text-main);">
                    </div>
                    <div>
                        <label for="order" style="font-size:0.75rem;color:var(--text-muted);display:block;margin-bottom:0.25rem;">Order</label>
                        <input id="order" type="number" name="order" min="0" value="{{ old('order', $part->order) }}"
                            style="width:100%;padding:0.5rem;border:1px solid var(--border-subtle);border-radius:0.4rem;background:rgba(15,23,42,0.9);color:var(--text-main);">
                    </div>
                    <div style="display:flex;gap:0.75rem;">
                        ...
                        <a href="{{ route('admin.constitution.parts') }}" class="dash-btn-ghost" style="text-decoration:none;padding:0.5rem 1rem;">Cancel</a>
                    </div>
                </div>
            </form>
        </section>
